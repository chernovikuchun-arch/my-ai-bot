import logging
import asyncio
import base64
import aiohttp
import os
import io
from PIL import Image # Rasmni o'lchamini o'zgartirish uchun
from aiogram import Bot, Dispatcher, F
from aiogram.filters import Command
from aiogram.types import Message, BufferedInputFile, ContentType
from aiohttp import web

# --- MA'LUMOTLAR ---
API_TOKEN = '8305734962:AAGFTI29uR8EI2jbgjWMIIM6x2MnrKImBNw'
STABILITY_KEY = 'sk-CJfzKSl4fHK3wV4lW3x8VRFyW3ZMXNpj5dgnojWB6He5vOac'

bot = Bot(token=API_TOKEN)
dp = Dispatcher()
user_data = {}

async def handle(request):
    return web.Response(text="Bot is resizing images for 1000 credits mode!")

@dp.message(Command("start"))
async def start_handler(message: Message):
    await message.answer("Salom! Endi rasm o'lchami bilan bog'liq muammo bo'lmaydi. Rasm yuboring!")

@dp.message(F.photo)
async def handle_photo(message: Message):
    user_data[message.from_user.id] = {'photo_id': message.photo[-1].file_id}
    await message.answer("Rasm qabul qilindi. Endi prompt yozing.")

@dp.message(F.text)
async def handle_prompt(message: Message):
    user_id = message.from_user.id
    if user_id not in user_data:
        await message.answer("Avval rasm yuboring!")
        return

    prompt = message.text
    photo_id = user_data[user_id]['photo_id']
    await message.answer("AI rasmni moslashtirmoqda va chizmoqda...")

    # 1. Telegramdan rasmni yuklab olish
    file = await bot.get_file(photo_id)
    file_url = f"https://api.telegram.org/file/bot{API_TOKEN}/{file.file_path}"
    
    async with aiohttp.ClientSession() as session:
        async with session.get(file_url) as resp:
            image_bytes = await resp.read()

        # 2. Rasm o'lchamini to'g'rilash (Resizing)
        img = Image.open(io.BytesIO(image_bytes))
        # Stability AI uchun eng yaxshi o'lcham - 768x768 yoki 1024x1024
        img = img.resize((768, 768)) 
        
        # Rasmni qayta byte ko'rinishiga o'tkazish
        byte_arr = io.BytesIO()
        img.save(byte_arr, format='PNG')
        resized_image_bytes = byte_arr.getvalue()

        # 3. Stability AI so'rovi
        url = "https://api.stability.ai/v1/generation/stable-diffusion-v1-6/image-to-image"
        headers = {
            "Accept": "application/json",
            "Authorization": f"Bearer {STABILITY_KEY}"
        }

        data = aiohttp.FormData()
        data.add_field("init_image", resized_image_bytes, filename="input.png")
        data.add_field("text_prompts[0][text]", prompt)
        data.add_field("image_strength", "0.45")
        data.add_field("cfg_scale", "7")
        data.add_field("steps", "30")

        async with session.post(url, headers=headers, data=data) as response:
            if response.status == 200:
                result = await response.json()
                img_base64 = result["artifacts"][0]["base64"]
                final_image = base64.b64decode(img_base64)
                await message.answer_photo(photo=BufferedInputFile(final_image, filename="res.png"), caption="Tayyor!")
                del user_data[user_id]
            else:
                err_data = await response.json()
                await message.answer(f"Xatolik: {err_data.get('message')}")

async def main():
    app = web.Application()
    app.router.add_get('/', handle)
    runner = web.AppRunner(app)
    await runner.setup()
    site = web.TCPSite(runner, '0.0.0.0', int(os.environ.get("PORT", 10000)))
    await asyncio.gather(site.start(), dp.start_polling(bot))

if __name__ == "__main__":
    asyncio.run(main())
